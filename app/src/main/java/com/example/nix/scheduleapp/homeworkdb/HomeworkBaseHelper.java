package com.example.nix.scheduleapp.homeworkdb;

import android.content.Context;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;
import static com.example.nix.scheduleapp.homeworkdb.HomeworkDbSchema.*;

/**
 * Created by Nix on 28.03.2017.
 */

public class HomeworkBaseHelper extends SQLiteOpenHelper{
    private static final int VERSION = 1;
    private static final String DATABASE_NAME = "homeworkBase.db";
    public HomeworkBaseHelper(Context context)
    {
        super(context, DATABASE_NAME, null, VERSION);
    }
    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table " + HomeworkTable.NAME + "(" +
                " _id integer primary key autoincrement, " +
                HomeworkTable.Cols.UUID + ", " +
                HomeworkTable.Cols.DESCR + ", " +
                HomeworkTable.Cols.SUBJECT_UUID +
                ")"
        );
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    }
}
